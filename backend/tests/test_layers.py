...
class TestCredentialsLayer:
    ...
    def test_pwz_prefix(self):
        result = _apply_credentials("Lekarz PWZ 1234567 wystawil.")
    ...
    def test_komornik(self):
        result = _apply_credentials("Komornik Sadowy nr 12/2022")
    ...
    def test_legitymacja_sluzb_sluzb(self):
        result = _apply_credentials("legitymacja sluzb owa nr AB123456")
...
class TestVerbalAmountLayer:
    ...
    def test_simple_verbal_amount(self):
        result = _apply_verbal("Kwota: sto dwadziescia zlotych")
        assert "sto dwadziescia zlotych" not in result
        assert "KWOTA_" in result

    def test_verbal_amount_with_fraction(self):
        result = _apply_verbal("suma trzy tysiace piecset zlotych 00/100")
        assert "trzy tysiace" not in result
        assert "KWOTA_" in result

    def test_verbal_amount_uppercase(self):
        result = _apply_verbal("STO ZLOTYCH")
        assert "KWOTA_" in result
    ...
    def test_kwota_counter_not_collide_with_existing(self):
        ...
        apply_verbal_amount_layer(s)
        assert "KWOTA_001" not in s.text or "dwiescie zlotych" not in s.text
        if "KWOTA_" in s.text:
